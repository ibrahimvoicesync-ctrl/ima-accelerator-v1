...
- `student_diy/*` follows the same editorial-restrained rules as `student/*` but amplifies scale — larger hero titles (`text-4xl md:text-6xl`) and the Work Tracker "stitch-blend" motif (monumental tabular-nums hero metric in `ima-primary`, filled-circle "stack of wins" for completed items, bordered-surface support cards instead of pastel bands).
