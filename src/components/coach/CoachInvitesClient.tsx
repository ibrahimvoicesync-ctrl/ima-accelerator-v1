...
export function CoachInvitesClient({ invites, magicLinks }: Props) {
  ...
  const [selectedRole, setSelectedRole] = useState<"student" | "student_diy">(
    "student",
  );
  ...
  const handleDeleteMagicLink = useCallback(async (link: MagicLinkItem) => {
    ...
    try {
      ...
      console.error("[CoachInvitesClient] delete magic link error:", err);
      toastRef.current({ type: "error", title: "Something went wrong" });
    }
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tabs — pill pattern to match owner invites */}
      <div
        ...
        <div className="bg-white border border-[#EDE9E0] rounded-[14px] p-6">
          ...
          <p className="mt-1 text-[12px] text-[#8A8474]">
            ...
            {selectedRole === "student_diy"
              ? "Student DIY (independent)"
              : "student assigned to you"}
            . No email restriction, no expiry.
          </p>
          <div className="h-px bg-[#EDE9E0] mt-4" aria-hidden="true" />
        ...
        {localMagicLinks.length === 0 ? (
          <div className="mt-4 bg-white border border-[#EDE9E0] rounded-[14px] p-6">
            <EmptyState
              ...
              description="Generate an invite link above to share with students."
            />
          </div>
        ) : (
          <ul className="mt-4 space-y-2" role="list">
