-- ============================================================================
-- Phase 61 (v1.8 F1): Student analytics outreach re-split — breaking jsonb shape.
-- Migration: 00033_fix_student_analytics_outreach_split.sql
--
-- Removes totals.total_emails (was mis-named + double-counted: SUM(brands + influencers))
-- and totals.total_influencers. Adds totals.total_brand_outreach (SUM(brands_contacted))
-- and totals.total_influencer_outreach (SUM(influencers_contacted)).
-- Signature (uuid, text, int, int) RETURNS jsonb is unchanged; function body is
-- copied verbatim from 00023 except the v_totals jsonb_build_object call.
-- Consumers (StudentAnalyticsTotals type, AnalyticsClient.tsx KPI cards, and
-- unstable_cache keys on both analytics pages) are updated in the same commit
-- series (Plans 02/03) so `npx tsc --noEmit` is the authoritative breakage
-- detector and the cache-key namespace switch is atomic. Supersedes 00023.
-- ============================================================================
...
--      total_emails key → REMOVED (was SUM(brands + influencers) — double-count bug)
--      total_influencers key → REMOVED (renamed)
--      total_brand_outreach key = SUM(COALESCE(brands_contacted,0))            ← NEW
--      total_influencer_outreach key = SUM(COALESCE(influencers_contacted,0))  ← NEW
...
BEGIN
  ...
  -- 5. Lifetime totals (ANALYTICS-02) — Phase 61 F1 re-split:
  --    total_brand_outreach + total_influencer_outreach replace total_emails + total_influencers.
  ...
  -- 6. Current-day streak — consecutive days ending today with a submitted
  --    daily_report. If no report today, streak = 0.
  WITH recent AS (
  ...
  -- 7. Outreach trend — per-ISO-week (brands vs influencers).
  --    Always weekly for consistency; clients with 7d range still get
  --    one or two weekly points which renders fine.
  SELECT COALESCE(jsonb_agg(row ORDER BY row->>'week_start'), '[]'::jsonb)
  INTO v_outreach_trend
  FROM (
    SELECT jsonb_build_object(
      'week_start', to_char(public.week_start(dr.date), 'YYYY-MM-DD'),
    ...
    WHERE dr.student_id = p_student_id
      ...
      AND (v_start_date IS NULL OR dr.date >= v_start_date)
    GROUP BY public.week_start(dr.date)
  ) s;

  -- 8. Hours trend — daily for 7d/30d, weekly for 90d/all.
  IF v_bucket_mode = 'day' THEN
    ...
    FROM (
      ...
      WHERE ws.student_id = p_student_id
        ...
        AND (v_start_date IS NULL OR ws.date >= v_start_date)
      GROUP BY ws.date
    ) s;
  ELSE
    SELECT COALESCE(jsonb_agg(row ORDER BY row->>'bucket'), '[]'::jsonb)
  ...
  -- 10. Roadmap progress rows (deadline math stays in TypeScript via getDeadlineStatus).
